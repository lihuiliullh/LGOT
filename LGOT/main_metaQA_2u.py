...
def calculate_accuracy(args, id2ent):
    ...
    hard_ans_new = {}
    # k is the query structure, v is true answrs
    ...
    # first add QTO results, then add LLMs results
    # if one answer both in LLM and QTO, it should receive a high score
    for k, v in logic_query_QTO.items():
        ...
        # read the answer from LLM, if answer in LLM, add score to them
        for a_ in llms_ans:
            if a_ not in ent2id:
                continue
            id = ent2id[a_]
        ...
        order = torch.argsort(candidate_by_QTO, dim=-1, descending=True) 
...
if __name__ == '__main__':
    ...
    args.data_path = "data/metaQA_2u_half"
    args.data_path_id_information = "kbc/src/data/metaQA_half"
    args.query_dir = 'data/metaQA_2u_half/test-queries.pkl'
