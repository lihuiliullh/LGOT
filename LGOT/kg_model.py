...
def neural_adj_matrix(model, rel, nentity, device, thrshd, adj_list):
    ...
    for s in range(0, nentity, bsz):
        t = min(nentity, s+bsz)
        h = torch.arange(s, t).to(device) # iterate each node id
...
class KGReasoning(nn.Module):
    ...
    # if want to combine chatGPT, seems like needs to input (entities, relation information) to build prompt
    def relation_projection(self, embedding, r_embedding, is_neg=False):
        ...
        for i in range(self.fraction):
            ...
            fraction_embedding, tmp_argmax = torch.max(fraction_embedding_premax, dim=1) # This is like beam search, only keep the best within s to t
            tmp_argmax = nonzero[tmp_argmax.squeeze()] + s # previous node, or parent node of current argmax result within s to t
            new_argmax = (fraction_embedding > new_embedding).to(torch.long).squeeze() 
            r_argmax = new_argmax * tmp_argmax + (1-new_argmax) * r_argmax  # previous node, or parent node of global argmax result
            new_embedding = torch.maximum(new_embedding, fraction_embedding)  # This is like beam search, only keep the best global
        
        # should call LLM here
        return new_embedding, r_argmax.cpu().numpy() # only store 
    ...
    def get_relation_embedding(self, r_idx):
        r_embedding = self.relation_embeddings[r_idx]
        return r_embedding


    # run the query and calculate answers
    def embed_query(self, queries, query_structure, idx):
        '''
        Iterative embed a batch of queries with same structure
        queries: a flattened batch of queries
        '''
        all_relation_flag = True # Lihui: this one means path query
        exec_query = []
        ...
        if all_relation_flag:
            if query_structure[0] == 'e':
                ...
                embedding.scatter_(-1, queries[:, idx].unsqueeze(-1), 1) # Tensor.scatter_(dim, index, src, reduce=None) 
